...
int MinCost(int person,int mask,int n,vector<vector<int>>& cost,vector<int>& dp){
    if(person>=n)return 0;
    if(dp[mask]!=-1)return dp[mask];
    int mini=INT_MAX;
    for(int i=0;i<n;i++){
        if(!(mask & (1<<i))){
            mini=min(mini,cost[person][n-i-1]+MinCost(person+1,mask |(1<<i),n,cost,dp));
        }
    }
    return dp[mask]=mini;
}
...
void solve() {
    ...
    int n=cost.size();
    vector<int> dp((1<<n),-1);
    // Keeping set {a1,a2,a3,..} as dp state ,so to keep track of set as state,
    // We will use bitmasking as state.
    // And there are many overlapping-sub problems, we will use dp with bitmasking.
    cout<<MinCost(0,0,n,cost,dp)<<endl;
}
