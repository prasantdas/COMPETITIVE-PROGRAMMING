...
bool IsOnOff(int ithbit,int subset){
    if(subset & (1<<ithbit)){
        return true;
    }
    else{
...
int UnsetBit(int ithbit,int num){
    return (num & (~(1<<ithbit)));
}
int ToggleBits(int ithbit,int num){
    return (num ^ (1<<ithbit));
}

void display(vector<int> set,int subset){
    int n=set.size();
    vector<int> ans;
    for(int i=0;i<n;i++){
        if(IsOnOff(i,subset)){
            ans.push_back(set[n-i-1]);
        }
    }
...
void solve() {
    //Write Here
    vector<int> set={5,4,3,1,9,5,3,1,8};
    // Let subset={5,3,1,8}; 
    // Bit representation = 101000011 (in binary) = 11 (in decimal)
    int subset=323;
    display(set,subset);
    cout<<endl;
    int a=5;
    cout<<UnsetBit(1,a)<<endl;
    cout<<ToggleBits(1,a)<<endl;
